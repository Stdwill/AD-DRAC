//账户复用，测试同时进行多次拍卖的交易确认延迟
// 用来测试同时进行多笔拍卖的测试文件
console.log(`Multiple auction with account reuse`);
...
contract("ModelReverseAuction: Concurrent Auctions Performance", (accounts) => {
  ...
  // 测试不同数量的并发拍卖
//   it("should test with 10 concurrent auctions", async () => {
//     await runConcurrentAuctionsTest(10, 20);
//   });
...
//   });
  ...
  it("should test with 1 concurrent auctions", async () => {
    await runConcurrentAuctionsTest(1, 20);
  });
...
//   });
  ...
  // 智能账户分配函数 - 复用现有账户
  function getBidderForAuction(auctionIndex, bidderIndex, totalBiddersPerAuction) {
    // 计算账户索引，循环使用可用账户
    const totalBiddersNeeded = totalBiddersPerAuction;
    const accountIndex = (auctionIndex * totalBiddersNeeded + bidderIndex) % allBidders.length;
    return allBidders[accountIndex];
  }
  ...
  async function runConcurrentAuctionsTest(numAuctions, biddersPerAuction) {
    ...
    console.log(`Available accounts: ${allBidders.length}, Required accounts: ${numAuctions * biddersPerAuction}`);
    console.log(`Account reuse ratio: ${(numAuctions * biddersPerAuction / allBidders.length).toFixed(2)}x`);
    ...
    for (const auction of createdAuctions) {
      for (let bidderIndex = 0; bidderIndex < biddersPerAuction; bidderIndex++) {
        // 使用智能账户分配，避免账户不足
        const bidder = getBidderForAuction(auction.index, bidderIndex, biddersPerAuction);
        ...
        const bidPromise = (async () => {
          ...
          return { auctionId: auction.auctionId, bidderIndex, bidder };
        })();
    ...
    console.log(`   All ${bidPromises.length} bids submitted using ${allBidders.length} accounts`);
      ...
      const refundPromise = (async () => {
        ...
        for (let i = 0; i < auction.biddersPerAuction; i++) {
          if (i !== auction.winningBidIndex) {
            const bidder = getBidderForAuction(auction.auctionId, i, auction.biddersPerAuction);
            losers.push(bidder);
          }
        }
  ...
  function printConcurrentResults(numAuctions, biddersPerAuction, results, totalTestTime) {
    ...
    console.log(`Account Strategy: Reused ${allBidders.length} accounts for all operations`);
    ...
    // 按操作类型分析延迟
    console.log("\n--- DELAY BREAKDOWN BY OPERATION ---");
    console.log("Operation".padEnd(25), "Avg Delay".padEnd(12), "Transactions");
    console.log("-".repeat(50));
    console.log("Auction Creation:".padEnd(25), `${creationStats.avgDelay.toFixed(1)}ms`.padEnd(12), creationStats.count);
    console.log("Bid Submission:".padEnd(25), `${bidStats.avgDelay.toFixed(1)}ms`.padEnd(12), bidStats.count);
    console.log("Auction Finalize:".padEnd(25), `${finalizationStats.avgDelay.toFixed(1)}ms`.padEnd(12), finalizationStats.count);
    console.log("Payment:".padEnd(25), `${paymentStats.avgDelay.toFixed(1)}ms`.padEnd(12), paymentStats.count);
    console.log("Refund:".padEnd(25), `${refundStats.avgDelay.toFixed(1)}ms`.padEnd(12), refundStats.count);

    // 成本估算
    const gasPriceInGwei = 1;
    const ethPriceInUSD = 200;
    const totalCostInUSD = (totalGas * gasPriceInGwei * 1e9) / 1e18 * ethPriceInUSD;
    const costPerAuction = totalCostInUSD / numAuctions;
    const costPerTransaction = totalCostInUSD / totalTransactions;
    ...
    console.log(`Cost per transaction: $${costPerTransaction.toFixed(4)}`);
